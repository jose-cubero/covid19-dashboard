...
def get_world_pop(country_list=[], region_list=[], continent_list=[]):
    ...
    df = df.rename(columns={'Country_Area': 'Country'})
...
def get_extended_wide():
    df_full = get_world_pop()
    
    # regions = df_full.groupby(["UN_Region"], as_index=False, group_keys=False).sum()
    regions = df_full.groupby(["UN_Region"], as_index=False).agg({'Population_2019': 'sum', 'Continent': 'first'})

    return pd.concat([df_full,  regions], ignore_index=True, copy=False)
